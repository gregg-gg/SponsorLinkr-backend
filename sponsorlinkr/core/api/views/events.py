...
from sponsorlinkr.core.api.serializers import CompanySerializer, EventSerializer, POCSerializer
...
class EventView(ModelViewSet):
    ...
    def delete(request, event_id):
        ...
        if request.user != event.user:
            return Response(
                {"detail": "You do not have permission to delete this event."}, status=status.HTTP_403_FORBIDDEN
            )
...
class FetchCompaniesForEvent(APIView):
    ...
    def post(request, event_id):
        ...
        companies = Company.objects.exclude(id__in=event.contacted_companies).order_by(
